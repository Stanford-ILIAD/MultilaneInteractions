...
class Visualizer(object):
    def __init__(self, dt=0.5, fullscreen=False, name='unnamed', iters=1000, magnify=1.):
        ...
        self.cntrl_loop_per_midlevel_iter = int(self.dt_midlevel/self.dt)
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.D:
            self.reset()
        if symbol == key.S:
            data_filename = 'data/%s-%d.pickle'%(self.name, int(time.time()))
    ...
    def output_loop(self, _):
        ...
        # output loop is equivalent to control loop
        self.cntrl_loop_counter += 1
    ...
    def run(self, filename=None, pause_every=None):
        ...
        # if the midlevel exists, schedule it
        if self.midlevel_exists:
            pyglet.clock.schedule_interval(self.midlevel_loop, self.dt_midlevel)
